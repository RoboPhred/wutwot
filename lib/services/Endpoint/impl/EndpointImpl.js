...
let EndpointImpl = class EndpointImpl {
    ...
    _createThingsRouter() {
        ...
        router.get("/", (ctx, next) => {
            ctx.body = this._thingManager.things.map(x => (Object.assign({}, this._getRestThing(x), { href: router.url("get-thing", x.id) })));
            next();
        });
...
EndpointImpl = __decorate([
    ...
    __param(0, microinject_1.inject(ThingManager_1.ThingManager))
], EndpointImpl);
exports.EndpointImpl = EndpointImpl;
function applyRouter(target, source) {
