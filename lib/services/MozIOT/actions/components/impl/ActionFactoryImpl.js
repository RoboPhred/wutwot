...
let ActionFactoryImpl = class ActionFactoryImpl {
    constructor(_actionRequestFactory, _actionRequestRepository) {
        ...
        this._thingActionIdMappers = new Map();
    }
    createAction(action, thingId, owner) {
        let idMapper = this._thingActionIdMappers.get(thingId);
        if (idMapper == null) {
            idMapper = new utils_1.IdMapper();
            this._thingActionIdMappers.set(thingId, idMapper);
        }
        const id = idMapper.createId(action.label);
        return new ThingActionImpl_1.ThingActionImpl(action, id, thingId, owner, this._actionRequestFactory, this._actionRequestRepository);
    }
};
